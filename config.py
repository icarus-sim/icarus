...
The specific instances of parameters supported are as follows:
 ...
 * workload:
    * name:
       * STATIONARY -> stationary Zipf workload
       * GLOBETRAFF -> synthetic workload generated by GlobeTradd
    * args:
        * For STATIONARY:
           ...
           * rate: requests rate
        * For GLOBETRAFF:
           * content_file: the path to a GlobeTraff content file
           * request_file: the path to a GlobeTraff request file
 * content_placement:
    * name:
 ...
 * cache_placement:
    ...
    * args
       ...
       * For CONSOLIDATED
          * spread: The fraction of top contrality nodes on which caches are deployed (optional, default: 0.5)
 * strategy:
    * name:
       ...
       * LIRA_LCE        ->  LIRA with LCE caching
       * LIRA_CHOICE     ->  LIRA with random choice caching
       * LIRA_PROB_CACHE ->  LIRA with ProbCache caching
    * args:
       * For PROB_CACHE
 ...
 * desc: string describing the experiment (used to print on screen progress information)
...
# Default experiment values, i.e. calues shared by all experiments
